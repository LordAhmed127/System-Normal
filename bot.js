...
if (message.content.startsWith(prefix + 'help')) { 
    let pages = [`.
        ***__اوامر عـامـه__***
**
${prefix}Server 的 معلومات سيرفر
${prefix}Bot 的 معلومات بوت
${prefix}invent 的 عداد دعواتك فى سيرفر
${prefix}cal 的 الالى حساب رياضيه
${prefix}line 的 حط كلام بشكل حلوه
${prefix}decoration 的 زخارف كلام الى انت عويز
${prefix}room 的 قال كام روم فى سيرفر واسم
${prefix}Emoji 的 طلع قائمه ايموجات الى فى سيرفر
${prefix}avatarServer 的 صور سيرفر
${prefix}avatar 的 صورتك وصور الى تمنشين
${prefix}ser 的 بحث على عضو بس الازم اول حرف من اسم
${prefix}Contact 的 ترسال شى الى صاحب بوت
${prefix}clore 的 ظهار الك قائمه الوان
${prefix}clo 的 عطاك لون
${prefix}id 的 لمعرفة معلومات عنك
**
  `
,`
        ***__اوامــر الادره__***
**
${prefix}bc 的 تسوى بردكاست نشره
${prefix}Addrole @user <اسم رتب> 的 عطاء رتب الى عضو
${prefix}move @user <rank> 的 سحب حد الى روم صوتى
${prefix}hide 的 خفاء شات
${prefix}show 的 ظهار شات
${prefix}del 的 مسح شات يعداد
${prefix}dela 的 مسح شات كله
${prefix}mute @user <وقت ميوت> 的 عطاء ميوت الى عضو
${prefix}unmute @user 的 فك ميوت عن عضو
${prefix}kick @user <سبيب طرد> 的 الى طرد عضو من سيرفر 
${prefix}ban @user <سبيب بنيدة> 的 الى حظر عضو من سيرفر
${prefix}cl 的 الى اقفل شات
${prefix}op 的 الى فتح شات
**
   `,`
**      
         ***__اوامــر الالعاب__***
...
**
   `,`
**  
         ***__ Lord_Ahmed#7241 ان اعجبك البوت و ارت مثلة فتواصل مع  __***
**    
`]
    let page = 1;
...
client.on('message', function(msg) {
 ...
 client.on('message', message => {
    ...
    client.on('message', message => {
   if(message.content.startsWith(prefix + "invent")) {
...
client.on('message', message => {
    if(message.content == '$vip-servers') {
             if(!message.author.id === '530403193130057740') return;
    var gimg;
    var gname;
    var gmemb;
    var gbots;
...
client.on('message', async msg => { // eslint-disable-line
	...
	//by ,$ Lord_Ahmed#72414777 'CODES SERVER'
...
//by ,$ Lord_Ahmed#72414777 'CODES SERVER'
	if (command === `play`) {
		...
		if (url.match(/^https?:\/\/(www.youtube.com|youtube.com)\/playlist(.*)$/)) {
			...
			try {//by ,$ Lord_Ahmed#72414777 'CODES SERVER'
...
//by ,$ Lord_Ahmed#72414777 'CODES SERVER'
		...
		return undefined;
	} else if (command === `stop`) {//by ,$ Lord_Ahmed#72414777 'CODES SERVER'
		if (!msg.member.voiceChannel) return msg.channel.send('أنت لست بروم صوتي .');
		...
		serverQueue.volume = args[1];//by ,$ Lord_Ahmed#72414777 'CODES SERVER'
...
//by ,$ Lord_Ahmed#72414777 'CODES SERVER'
	...
	} else {//by ,$ Lord_Ahmed#72414777 'CODES SERVER'
...
function play(guild, song) {
	...
	if (!song) {//by ,$ Lord_Ahmed#72414777 'CODES SERVER'
		serverQueue.voiceChannel.leave();
		queue.delete(guild.id);
		return;//by ,$ Lord_Ahmed#72414777 'CODES SERVER'
	}//by ,$ Lord_Ahmed#72414777 'CODES SERVER'
	console.log(serverQueue.songs);
//by ,$ Lord_Ahmed#72414777 'CODES SERVER'
	const dispatcher = serverQueue.connection.playStream(ytdl(song.url))
		.on('end', reason => {//by ,$ Lord_Ahmed#72414777 'CODES SERVER'
			if (reason === 'Stream is not generating quickly enough.') console.log('Song ended.');
			else console.log(reason);
			serverQueue.songs.shift();//by ,$ Lord_Ahmed#72414777 'CODES SERVER'
			play(guild, serverQueue.songs[0]);
		})//by ,$ Lord_Ahmed#72414777 'CODES SERVER'
		.on('error', error => console.error(error));//by ,$ Lord_Ahmed#72414777 'CODES SERVER'
	dispatcher.setVolumeLogarithmic(serverQueue.volume / 5);//by ,$ Lord_Ahmed#72414777 'CODES SERVER'

	serverQueue.textChannel.send(`بدء تشغيل : **${song.title}**`);
}//by ,$ Lord_Ahmed#72414777 'CODES SERVER'

const adminprefix = "$vip";//by ,$ Lord_Ahmed#72414777 'CODES SERVER'
client.on('message', message => {//by ,$ Lord_Ahmed#72414777 'CODES SERVER'
  var argresult = message.content.split(` `).slice(1).join(' ');//by ,$ Lord_Ahmed#72414777 'CODES SERVER'
    if (!devs.includes(message.author.id)) return;//by ,$ Lord_Ahmed#72414777 'CODES SERVER'
    message.reply("**`only for Bot Owner`**")
if (message.content.startsWith(adminprefix + 'setgame')) {//by ,$ Lord_Ahmed#72414777 'CODES SERVER'
...
client.on("message", message => {
 if (message.content === `${prefix}`) {
  const embed = new Discord.RichEmbed() //by ,$ Lord_Ahmed#72414777 'CODES SERVER'
      .setColor("#000000")//by ,$ Lord_Ahmed#72414777 'CODES SERVER'
      .setDescription(`
...
${prefix}queue ⇏ لمعرفة قآئمة التشغيل
 `)//by ,$ Lord_Ahmed#72414777 'CODES SERVER'
   message.channel.sendEmbed(embed)//by ,$ Lord_Ahmed#72414777 'CODES SERVER'
 ...
 client.on('ready', () => {
    console.log(`Logged in as ${client.user.tag} !`);
          client.user.setActivity("Trye:$help or Call:Lord_Ahmed#7241",{type: 'WATCHING'});
